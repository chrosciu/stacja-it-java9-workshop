...
//                .anyOf(CompletableFuture.supplyAsync(LongRunningChecks::check15sec), timeoutAfter(TIMEOUT, TimeUnit.SECONDS));
        ...
        CompletableFuture<LongRunningChecks.STATUS> fastService = CompletableFuture
                .supplyAsync(LongRunningChecks::check5sec)
                .completeOnTimeout(LongRunningChecks.STATUS.TIMEOUT, TIMEOUT, TimeUnit.SECONDS);

        CompletableFuture<LongRunningChecks.STATUS> mediumService = CompletableFuture
                .supplyAsync(LongRunningChecks::check10sec)
                .completeOnTimeout(LongRunningChecks.STATUS.TIMEOUT, TIMEOUT, TimeUnit.SECONDS);

        CompletableFuture<LongRunningChecks.STATUS> slowService = CompletableFuture
                .supplyAsync(LongRunningChecks::check15sec)
                .completeOnTimeout(LongRunningChecks.STATUS.TIMEOUT, TIMEOUT, TimeUnit.SECONDS);
